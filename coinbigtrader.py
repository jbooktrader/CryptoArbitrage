...
import threading
import csv
from COINBIG.coinbigapi import CoinBig
import datetime
import sys
##############################参数设置#################################################
# API KEY
...
threads = []
# 实例化COINEX接口
coinbig = CoinBig(apikey, secretkey)
refresh_flag = 0
        ...
        try:
            ...
            if (accountbalance - startamount >= minamount * 2):
                ...
                while (count < 5):
                    try:
                        count = count + 1
                        orderid = coinbig.order_limit(pair,'sell',str(amount),str(round(bid1 * 0.995, 2)))
                        if (orderid > 0):
                ...
                while (count < 5):
                    try:
                        count = count + 1
                        orderid = coinbig.order_limit(pair,'buy',str(amount),str(round(ask1 * 1.005, 4)))['id']
                        if (orderid > 0):
                            nowTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
def cancelOrdersThread():
    while (True):
        time.sleep(120)
        try:
            orderlist = coinbig.order_pending(pair)
            ...
            if(size > 0):
                ...
                for item in orderlist['data']:
                    try:
                        orderid = item['id']
                        coinbig.order_pending_cancel(pair,orderid)
                        count = count +1
                        time.sleep(0.5)
                    except Exception as ex:
                        print(ex)
...
def autoSell():
    while (True):
        ...
        try:
            ...
            if (cetamount > 1):
                bid = float(coinbig.market_depth('CBUSDT')['bids'][0][0])
                price = round(bid * 0.95, 4)
                amount = round(cetamount, 0) - 1
                print(coinbig.order_limit('CBUSDT', 'sell', amount, price)['id'])
...
def strategy():
    while(True):
        try:
            ...
            # time.sleep(2)
            start1 = time.time()
            res = coinbig.market_depth(pair)
            print(res)
            end1 = round(time.time() - start1,3)
            bid1 = float(res['data']['bids'][0][0])
            ask1 = float(res['data']['asks'][0][0])
            print('盘口买价:' + str(bid1) + '  盘口卖价:' + str(ask1))
            ...
            if (bid1 > 0 and ask1 > 0 and (ask1-bid1)>= minspread):
                ...
                start2 = time.time()
                buyorderid = coinbig.order_limit(pair,'buy',minamount,round(price + slippage, 2))['id']
                if(buyorderid > 0):
                    ...
                    while (count < 5):
                        try:
                            count = count + 1
                            sellorderid = coinbig.order_limit(pair, 'sell', minamount,round(price - slippage, 2))['id']
                            if (sellorderid > 0):
